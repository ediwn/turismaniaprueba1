...
<?php
   ...
   $action = (isset($_REQUEST["action"])&& $_REQUEST["action"] !=NULL && $_REQUEST["txt_buscar_area"])?$_REQUEST["action"]:"";
   ...
   if($action == "")
    {
      ...
      <a href="../vista/vista_listar_area.php"> Volver al listado </a>
      <?php
    }
  ...
  if($action == "ajax"){
    ...
    $txt_buscar_area=$_REQUEST["txt_buscar_area"];
    
   

    echo "<label>".$txt_buscar_area." !! </label> </h4>";
    ?><a href="../vista/vista_listar_area.php"> Volver al listado </a> <?php
    ...
    $count_query = mysqli_query($con,"SELECT count(*) AS numrows FROM area WHERE nombre_area LIKE '%$txt_buscar_area%' ");if ($row= mysqli_fetch_array($count_query)){$numrows = $row["numrows"];}
    $total_pages = ceil($numrows/$per_page);
    $reload = "../vista/vista_listar_area.php";

    //consulta principal para recuperar los datos
    $query = mysqli_query($con,"SELECT * FROM area WHERE nombre_area LIKE '%$txt_buscar_area%'  order by ID_area LIMIT $offset,$per_page");
    
    if ($numrows>0){
      ?>
    <table class="table table-bordered">
        <thead><th>nombre_area</th><th class="col-lg-2" aling="center"> </th> </tr>
      </thead>
      <tbody>
      <?php
       while($row = mysqli_fetch_array($query)){$ID_area=$row['ID_area'];?> <tr><td><?php echo $row["nombre_area"];?></td><td><button type="button" class="btn btn-info btn-xs" data-toggle="modal" data-target="#myModal" onclick="btn_editar_area('<?php echo $ID_area;?>')"> Editar </button> 
