<?php
    ...
    $count_query   = mysqli_query($con,"SELECT count(*) AS numrows FROM user");
    if ($row= mysqli_fetch_array($count_query)){$numrows = $row["numrows"];}
    $total_pages = ceil($numrows/$per_page);
    $reload = "../vista/vista_listar_user.php";

    //consulta principal para recuperar los datos
    $query = mysqli_query($con,"SELECT * FROM user order by ID_user LIMIT $offset,$per_page");
    
    if ($numrows>0){
      ?>
    <table class="table table-bordered">
        <thead>
        <tr><th>correo</th><th>password</th><th>permiso</th>
      ...
      <?php
       while($row = mysqli_fetch_array($query)){
        $ID_user=$row['ID_user'];
        ?>
        <tr><td><?php echo $row["correo"];?></td><td><?php echo $row["password"];?></td><td><?php echo $row["permiso"];?></td><td><button type="button" class="btn btn-info btn-xs" data-toggle="modal" data-target="#myModal" onclick="btn_editar_user('<?php echo $ID_user;?>')"> Editar </button> 
